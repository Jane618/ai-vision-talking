...
| 发送即取帧 | 用户发送文字或语音消息时，自动抓取当前摄像头画面作为本轮视觉输入 |
| 消息缩略图 | 带图用户消息旁展示本轮实际发送给 AI 的缩略图，点击可全屏查看 |
...
| 依赖 | 用途 |
|---|---|
| `react`、`react-dom` | 构建前端组件、状态更新和页面渲染 |
| `@capacitor/core`、`@capacitor/app` | 为移动端原生打包提供运行时桥接 |
| `@capacitor-community/speech-recognition` | 在 Capacitor 环境中调用原生语音识别 |
| `@capacitor-community/text-to-speech` | 在 Capacitor 环境中调用原生文本转语音 |
| `@capacitor/haptics`、`@capacitor/keyboard`、`@capacitor/status-bar` | 移动端触感、键盘和状态栏适配 |

### 前端开发依赖

| 依赖 | 用途 |
|---|---|
| `vite`、`@vitejs/plugin-react` | 前端开发服务器和生产构建 |
| `typescript` | 类型检查和编译 |
| `@types/react`、`@types/react-dom`、`@types/dom-speech-recognition` | TypeScript 类型声明 |
| `@capacitor/cli` | Capacitor 项目同步、构建和平台管理 |
...
| 能力 | 来源 | 用途 |
|---|---|---|
| 摄像头访问 | 浏览器 `navigator.mediaDevices.getUserMedia` | 获取用户摄像头视频流 |
| Canvas 图像处理 | 浏览器 Canvas API | 抽帧、缩放、采样和 JPEG 压缩 |
| 语音识别 | Web Speech API / Capacitor Speech Recognition | 将用户语音转成文本 |
| 语音播报 | Web Speech Synthesis / Capacitor Text to Speech / 可选火山引擎 TTS | 播放 AI 回复 |
| 多模态模型 | 火山引擎 Ark / 豆包多模态模型 | 图片理解、文本生成和流式回复 |
...
| 模块 | 原创实现内容 |
|---|---|
| 多模态对话流程 | 将文字/语音输入、发送即取帧、后端流式调用和前端逐字展示组合为一条完整对话链路 |
| 本地抽帧与图像处理 | 实现摄像头抽帧、最长边缩放、JPEG 压缩、复杂度分析和自适应质量选择 |
| 画面变化检测 | 基于低分辨率采样、平均差、变化像素比例、边缘变化和局部块变化判断画面变化 |
| 用户消息缩略图 | 保存每轮实际发送给 AI 的画面，并在用户消息旁提供缩略图和全屏查看 |
| 场景预设与质量模式 | 为不同使用场景组合提示词、图像参数、摘要阈值和成本策略 |
| 流式回复体验 | 封装 SSE 客户端，增量更新 AI 消息，并将回复按句切分后加入播报队列 |
| 语音状态控制 | 处理 ASR 自动发送、播报期间禁用录音、迟到识别事件过滤和输入框同步 |
| 对话摘要与成本统计 | 按 token 阈值压缩历史上下文，累计 prompt/completion token、节省 token 和估算费用 |
| 主题与响应式界面 | 实现深浅色主题、月亮/太阳拨动开关、星空/蓝天背景和双栏响应式布局 |
...
| 本地预览帧与发送帧分离 | 已实现 | 右下角缩略图只用于本地预览，对话发送时重新抓取当前画面 |
